...
const setCount = async (req, res) => {
    try {
        ...
        if (typeof count === 'number' && count >= 0 && ((count) % 1 === 0)) {
            response = await Starship.findOneAndUpdate( { _id: id },
                { $set : {'count' : count } },
                { new: true })
        } else  {
        ...
        res.json({
            count: response.count,
            message: 'Total count for this specific starship is set to'+ response.count,
            name: response.name
        })
...
const incDecCount = async (req, res, next) => {
    try {
        ...
        if (operator === 'inc') {
            ...
            job = 'decremented'
            response = await Starship.findById(id)
            if (response.count > 0) {
                response = await Starship.findOneAndUpdate( { _id: id },
                    { $inc : {'count' : -1 } },
                    { new: true })
        ...
        res.json({
            count: response.count,
            message: 'Total count for this specific starship is '+ job +' to '+ response.count,
            name: response.name
        })
